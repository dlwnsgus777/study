...
빌딩 블록 -> 도메인의 개념을 코드로 옮기기 위한 가이드
- 엔티티를 통해 도메인을 객체로
- 애그리게이트를 통해 객체의 라이프싸이클을 관리하기 위해

도메인 모델 -> 코드를 짜기 편하고 코드를 잘 변경하기 쉬운 구조
- 도메인 코드를 짜기 편하게 각색하는 것

실무에 무언가를 할 때는 다른 개념을 섞어서 사용하게 된다.
- DDD만 적용해야지 -> 가능하지 않다.
- 정상적인 경우를 벗어나면 다른 개념과 트레이드 오프를 해야한다.

DDD는 복잡한 걸 단순화하는 것에 포커스가 맞춰져 있다.

DDD에서는 요구사항과 불변식을 섞어서 설계를 한다.

객체지향은 기능 요구사항을 가지고 객체끼리 협력하는 것

DDD는 기능 요구사항에 불변식을 섞어서 전체 코드의 구조를 짜는 것
- 구조의 단위는 애그리거트

DDD로 코드를 짠다
- 요구사항
- 요구사항으로 상태가 바뀔 때에 사용되는 불변식

엔티티와 값 객체를 따로보면 이해가 잘 안된다.

엔티티 
- 코드를 짜려고 하는데 이걸 엔티티로 잡는게 코드를 짜기 훨씬 쉽겠어 -> 이런 관점이다.
- 연속성이나 식별성
...
엔티티
- 사람이 ㅁ낳은데 이 사람 중 특정한 누군가를 찾고 싶은데 한명을 식벽할 값이 있어야할 때 
- 어떤 형태인든 무조건 추적해야할 때
- 값이 바뀌더라도 그 값으로 트래킹을 해야할 때

값 객체
- 여러 값 객체 중에 하나만 검색해서 사용해도 될 때
- 엔티티를 단순화하려고 사용한다
- 모든 값을 다 비교할 때

엔티티 안에 값 객체를 사용할 때는 테이블 안에 값을 다 미뤄넣는다.

가변 객체는 추적하기가 어렵다.

실무에서는 엔티티를 막 짠다
- 하다보면 속성이 붙는다
- 코드가 점점 늘어나고 코드를 이해하기 어려워진다
- 이때 값 객체를 사용한다.
  - 값 객체를 떼간다.
- 비슷한 로직을 값 객체이 몰아놓는다.
...
엔티티를 개발하다보니 점점 복잡해진다.
- 이럴 때 VO를 사용한다.
- 엔티티가 복잡할 때 VO를 사용하면 간단해진다.
- 뒤의 로직이 복잡하더라도 변경의 부분을 찾을 수 있다.

값만 필요하면 VO, 추적이 필요하면 엔티티를 사용한다.

`Part2` 는 코드레벨에 대한 이야기이다.
- 도메인에 대한 이해가 높을 때 엔티티와 값 객체를 가지고 표현하면 유지보수하기 좋아요 의 내용이다.

객체지향을 할 때 DDD의 개념을 적절히 합치면 좋다.

여러가지 개념을 적절한 상황에 적절하게 적용하는게 중요하다.

가변 객체의 상태를 불변객체로 뜯어내면 코드의 상태를 추적하기 좋다.

### 연관관계와 애그리게이트

엔티티와 VO는 애그리게이트 단위로 묶여다닌다.

책임주도 설계롤 객체를 설계하고 -> 기능 요구사항
계약의 의한 설계 -> 불변식

기능 요구사항과 불변식을 통해 애그리게이트를 만든다.
...
애그리거트 루트는 식별이 가능해야함 -> 전역 식별성
- 루트 안에 있는 엔티티는 지역 식별성을 가져도 된다.

애그리거트 루트 안에 모든 로직이 들어간다.
- 자기에 대한 상태를 바꾸는 것 뿐만 아니라 안에 있는 엔티티의 상태를 바꾸는 것도 포함

`has-a` 관계가 아닌 불변식을 기준으로 애그리거트 경계가 정의된다.

애그리거트는 트랜잭션, 데이터 저장/조회의 단위

애그리거트 루트 내부의 로직은 직관적을 수 있도록 심플해야 한다.

애그리거트를 저장하는 건 레포지토리의 역할

레포지토리 -> 애그리거트 라이프 싸이클을 관리
- 애그리거트와 레포지토리는 같이 다닌다.

레포지토리
- 데이터 저장소에 저장하는 걸 캡슐화함
- DB의 관점이 아니다.

애그리거트와 애그리거트 사이의 연결에 필요

리파지토리를 이해하려면 연관관계를 알아야한다.

연관관계
- A라는 객체가 있을 때
- A를 통해서 B 객체에 접근할 수 있다면 연관관계가 있는 것

구현과 개념은 다르다.

연관관계는 개념이다.
- 여러가지 방식으로 구현할 수 있다는 것이 됨

연관관계는 단순화 시킬수록 좋다.
- 도메인 모델이 도메인을 그대로 코드로 가져오지 않음
- 개발하기 좋은 방식으로 도메인을 재가공
  - 그 중 하나가 연관관계
...
**결국에는 코드 짜기에 좋은 걸 선택해야함**

제3의 객체를 통해 탐색
- 리포지토리의 역할

객체참조 -> 애그리거트 내부
어이디를 사용한 참조 -> 애그리거트 외부
-> 리파지토리를 통해 외부 애그리거트를 가져옴

레이지로딩 -> 어디까지 읽어야할지 몰라서 씀

애그리거트 경계 정하기
애그리거트는 기능 구현 + 불변식을 고려

비즈니스 룰은 개발하다보면 자연스럽게 정의하게 되어있음

객체 연관관계의 방향은 데이터의 방향이 아니라 행위의 방향

최대한 단방향으로 가고 예외 케이스를 끼워 맞추자.

제약 사항이 바뀌면 애그리거트 경계도 바뀐다.

애그리거트를 짤 때 어떤 방식으로 짜고 있었는지가 중요함.
- 엔티티와 값 객체 중 어느 것을 골랐을 때 일관되게 짜야한다.

DB와 값 객체는 다르다.

ORM과 DDD의 엔티티는 다르다.
- JPA 엔티티를 DDD의 엔티티처럼 개발할 수는 있다.

JPA 리포지토리를 자기의 리포지토리에 상속받는 거는 굳이 상관없다.
- 결합도 -> 결합도는 바꾸려고 할 때 문제가 된다.

개발의 순수성 -> 공부하기 위한 영역
더 중요한 건 실용적으로 응용할 때

애그리거트를 만들 때 불변식을 맞춘 상태로 만들고 싶다면 -> 팩토리를 사용하자.

도메인 서비스 -> 어떤 로직을 객체에 넣고 싶은데
- 아무리봐도 상태가 필요없음
- 여러 객체를 묶어서 처리해야함
- 어디에 넣을지 모르겠음
- 이럴때 서비스를 만들자.
...
- 도메인 로직
  - 기술적인 것을 덜어내고
  - 컴퓨터가 없다고 생각하고 이 로직을 설명할 수 있어야함
- 도메인 로직을 처리하기 위해 사용하는 워크 플로우
  - 어플리케이션 서비스라고 함

DDD는 전부 다 하려고 하지말고 필요한 개념을 가져와 사용하는게 좋음

객체지향을 할 때 DDD의 원칙을 가지고 와서 코드에 녹이는 것

---

## 그 외 질문타임

객체지향을 하려고 할 때 orm을 안쓰면 힘들다.

ddd 레포지토리 구현 예제를 보면 거의 ORM인데 orm없이 ddd 레포지토리 구현은 어떻게 생각하시는지

→ orm아니면 ddd가 적합하지 않은건가?

- 적합하지는 않은데 굉장히 힘들다
- 무언가를 짜는 건 문제가아님
  - 결국에 짜고 나서 유지보수하는 게 중요함
  - DDD는 ORM 없이도 가능하지만 굉장히 유지보수하기 힘들 것
    - 무조건 객체지향을 해야 DDD 인가? 그건 아니기 때문에 적절히 잘 선택해야함

객체지향으로 코드를 짜려고해도 ORM없이 가능할지? 에 대한 고민?

- 가능은하지만 순수하게 객체지향 적으로 코드를 짜고 싶다면 ORM 을 사용해라 라고 말하고싶음
- ORM 없이도 가능은 하지만 굉장히 어려울 것
  - 유지보수가 늘어나고 스트레스가 쌓일 것

DDD는 기본적으로 코드의 수정을 줄이려고함
...
- 어떤 책을 봄
  - 개념이 나옴
    - 레퍼런스를 쫓아가서 개념을 다 봄
- 그 책에 있는 내용을 제대로 이해해야하기 위해
  - 다른걸 다 쫓아가서 이해함
    - 개념은 다른 개념의 종합
      - 다른 개념을 다 내꺼로 만들어서 이해해야함
    - 본인이 이해한 개념으로 강의를 설명
  - 내가 코드로 짜본다
    - 책을 보면 한 줄에 많은 내용이 내포되어 있다.
    - 결국 적용해봐야 안다.
- 어떤 책이나 어떤 개념이 하나로 끝나지는 않기 때문에 최대한 많이 깊이 있게 내 머리속에 높여야함
  - 실제로 적용해봐야 암
- 개발적으로 성장했을 때는 공부했던 걸 녹여보려 노력했었음
- 최대한 깊게 보려고 노력함
  - 주니어때 본 책을 시니어때 한번 보는 것도 도움됨

코드나 원칙을 설명할 때 설명할 게 말이 길어지지 않으면 단순한 것
